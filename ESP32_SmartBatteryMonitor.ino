...
void recoverI2C() {
  ...
  // Configure pins as output and pull high
  ...
  // Send 9 clock pulses on SCL while SDA is high (to release stuck slave)
  for (int i = 0; i < 9; i++) {
  ...
  // Release pins back to input (high-impedance) for I2C restart
  pinMode(sda_pin, INPUT_PULLUP);  // Better: use internal pull-ups
  ...
  Wire.setClock(50000);  // Safe 50 kHz for smart batteries

  i2cError = false;
  Serial.println("[I2C] Bus recovered with clock pulses (SDA=" + String(sda_pin) + ", SCL=" + String(scl_pin) + ")");
}
...
void updateBatteryData() {
  ...
  raw = readWord(CMD_TEMPERATURE); bat.temperature = (raw == 0xFFFF) ? -1 : (raw / 10.00) - 273.15; delay(50);
  ...
  <script>
    ...
    var updateData = function() {
      ...
      fetch('/data').then(r => r.json()).then(d => {
        ...
        document.getElementById('temp').innerText = (d.temp === null ? '--' : (d.temp / 1).toFixed(1));
  ...
  async function sendBatch() {
    ...
    resultDiv.innerHTML = `
      ...
      <table class="table table-sm table-striped">
        <thead class="table-dark"><tr><th>Cmd</th><th>Name</th><th>Hex</th><th>Dec</th><th>String</th><th>Change</th></tr></thead>
    ...
    for (let cmd = start; cmd <= end; cmd++) {
      ...
      let hexCell = row.insertCell(2);
      let decCell = row.insertCell(3);
      let strCell = row.insertCell(4);
    ...
    if (response.includes('Word Response')) {
      ...
      let nameMatch = response.match(/^Name:(.+)$/m);
      let cmdName = nameMatch ? nameMatch[1].trim() : "n/a";

      // Extract hex value
      let hexMatch = response.match(/0x([0-9A-F]+)/i);
      let hex = hexMatch ? hexMatch[1].toUpperCase() : '';
      let dec = hex ? parseInt(hex, 16) : 0;

      // Fill cells
      nameCell.textContent = cmdName;          // New: Name column
      hexCell.textContent = hex ? '0x' + hex : '';
      decCell.textContent = dec;
      strCell.textContent = '';                // Clear string for word response
      ...
      // Extract string after "String: "
      let strMatch = response.match(/String:\s*(.+)$/m);
      let str = strMatch ? strMatch[1].trim() : '';

      // Fill cells
      nameCell.textContent = cmdName;          // New: Name column
      hexCell.textContent = '';                // No hex for block
      decCell.textContent = '';                // No decimal for block
      strCell.textContent = str || '(empty)';

    } else {
      // No recognized format (error, no response, etc.)
      let nameMatch = response.match(/^Name:(.+)$/m);
      let cmdName = nameMatch ? nameMatch[1].trim() : "n/a";

      nameCell.textContent = cmdName;
      hexCell.textContent = response.trim();
      decCell.textContent = '';
      strCell.textContent = '';
    }
...
server.on("/cmd", HTTP_GET, [](AsyncWebServerRequest *request) {
  ...
  // Optional subcommand for ManufacturerAccess (0x00)
    ...
    if (isManufAccess && subcmd != 0) {
      cmdName = "ManufacturerAccess subcmd 0x" + String(subcmd, HEX);
    }

    // === Standard SBS Command Names ===
    switch (cmd) {
      case 0x00: cmdName = "ManufacturerAccess()"; break;
      case 0x01: cmdName = "RemainingCapacityAlarm()"; break;
      case 0x02: cmdName = "RemainingTimeAlarm()"; break;
      case 0x03: cmdName = "BatteryMode()"; break;
      case 0x04: cmdName = "AtRate()"; break;
      case 0x08: cmdName = "Temperature()"; break;
      case 0x09: cmdName = "Voltage()"; break;
      case 0x0A: cmdName = "Current()"; break;
      case 0x0B: cmdName = "AverageCurrent()"; break;
      case 0x0C: cmdName = "MaxError()"; break;
      case 0x0D: cmdName = "RelativeStateOfCharge()"; break;
      case 0x0E: cmdName = "AbsoluteStateOfCharge()"; break;
      case 0x0F: cmdName = "RemainingCapacity()"; break;
      case 0x10: cmdName = "FullChargeCapacity()"; break;
      case 0x11: cmdName = "RunTimeToEmpty()"; break;
      case 0x12: cmdName = "AverageTimeToEmpty()"; break;
      case 0x13: cmdName = "AverageTimeToFull()"; break;
      case 0x14: cmdName = "ChargingCurrent()"; break;
      case 0x15: cmdName = "ChargingVoltage()"; break;
      case 0x16: cmdName = "BatteryStatus()"; break;
      case 0x17: cmdName = "CycleCount()"; break;
      case 0x18: cmdName = "DesignCapacity()"; break;
      case 0x19: cmdName = "DesignVoltage()"; break;
      case 0x1A: cmdName = "SpecificationInfo()"; break;
      case 0x1B: cmdName = "ManufactureDate()"; break;
      case 0x1C: cmdName = "SerialNumber()"; break;
      case 0x20: cmdName = "ManufacturerName()"; break;
      case 0x21: cmdName = "DeviceName()"; break;
      case 0x22: cmdName = "DeviceChemistry()"; break;
      case 0x23: cmdName = "ManufacturerData()"; break;
      case 0x3C: cmdName = "CellVoltage4()"; break;
      case 0x3D: cmdName = "CellVoltage3()"; break;
      case 0x3E: cmdName = "CellVoltage2()"; break;
      case 0x3F: cmdName = "CellVoltage1()"; break;
      case 0x4A: cmdName = "BTPDischargeSet()"; break;
      case 0x4B: cmdName = "BTPChargeSet()"; break;
      case 0x4F: cmdName = "State-of-Health (SOH)"; break;
      case 0x50: cmdName = "SafetyAlert"; break;
      case 0x51: cmdName = "SafetyStatus"; break;
      case 0x52: cmdName = "PFAlert"; break;
      case 0x53: cmdName = "PFStatus"; break;
      case 0x54: cmdName = "OperationStatus"; break;
      case 0x55: cmdName = "ChargingStatus"; break;
      case 0x56: cmdName = "GaugingStatus"; break;
      case 0x57: cmdName = "ManufacturingStatus"; break;
      case 0x58: cmdName = "AFE Register"; break;
      case 0x59: cmdName = "MaxTurboPwr()"; break;
      case 0x5A: cmdName = "SusTurboPwr()"; break;
      case 0x5B: cmdName = "TURBO_PACK_R"; break;
      case 0x5C: cmdName = "TURBO_SYS_R"; break;
      case 0x5D: cmdName = "TURBO_EDV"; break;
      case 0x5E: cmdName = "MaxTurboCurr()"; break;
      case 0x5F: cmdName = "SusTurboCurr()"; break;
      case 0x60: cmdName = "Lifetime Data Block 1"; break;
      case 0x61: cmdName = "Lifetime Data Block 2"; break;
      case 0x62: cmdName = "Lifetime Data Block 3"; break;
      case 0x63: cmdName = "Lifetime Data Block 4"; break;
      case 0x64: cmdName = "Lifetime Data Block 5"; break;
      case 0x70: cmdName = "ManufacturerInfo"; break;
      case 0x71: cmdName = "DAStatus1"; break;
      case 0x72: cmdName = "DAStatus2"; break;
      case 0x73: cmdName = "GaugeStatus1"; break;
      case 0x74: cmdName = "GaugeStatus2"; break;
      case 0x75: cmdName = "GaugeStatus3"; break;
      case 0x76: cmdName = "CBStatus"; break;
      case 0x77: cmdName = "State-of-Health"; break;
      case 0x78: cmdName = "FilteredCapacity"; break;

    }

    // === Extended ManufacturerAccess Subcommand Names (bq20zxx/bq2084) ===
    if (isManufAccess && subcmd != 0) {
      switch (subcmd) {
        case 0x0001: cmdName = "Device Type"; break;
        case 0x0002: cmdName = "Firmware Version"; break;
        case 0x0003: cmdName = "Hardware Version"; break;
        case 0x0004: cmdName = "Instruction Flash Signature"; break;
        case 0x0005: cmdName = "Static DF Signature"; break;
        case 0x0006: cmdName = "Chemical ID"; break;
        case 0x0008: cmdName = "Static Chem DF Signature"; break;
        case 0x0009: cmdName = "All DF Signature"; break;
        case 0x0010: cmdName = "SHUTDOWN Mode"; break;
        case 0x0011: cmdName = "SLEEP Mode"; break;
        case 0x0013: cmdName = "AutoCCOffset"; break;
        case 0x001D: cmdName = "Fuse Toggle"; break;
        case 0x001E: cmdName = "PCHG FET Toggle"; break;
        case 0x001F: cmdName = "CHG FET Toggle"; break;
        case 0x0020: cmdName = "DSG FET Toggle"; break;
        case 0x0021: cmdName = "Gauging"; break;
        case 0x0022: cmdName = "FET Control"; break;
        case 0x0023: cmdName = "Lifetime Data Collection"; break;
        case 0x0024: cmdName = "Permanent Failure"; break;
        case 0x0025: cmdName = "Black Box Recorder"; break;
        case 0x0026: cmdName = "Fuse"; break;
        case 0x0027: cmdName = "LED DISPLAY Enable"; break;
        case 0x0028: cmdName = "Lifetime Data Reset"; break;
        case 0x0029: cmdName = "Permanent Fail Data Reset"; break;
        case 0x002A: cmdName = "Black Box Recorder Reset"; break;
        case 0x002B: cmdName = "LED Toggle"; break;
        case 0x002C: cmdName = "LED Display Press"; break;
        case 0x002D: cmdName = "CALIBRATION Mode"; break;
        case 0x002E: cmdName = "Lifetime Data Flush"; break;
        case 0x002F: cmdName = "Lifetime Data SPEED UP Mode"; break;
        case 0x0030: cmdName = "Seal Device"; break;
        case 0x0035: cmdName = "Security Keys"; break;
        case 0x0037: cmdName = "Authentication Key"; break;
        case 0x0041: cmdName = "Device Reset"; break;
        case 0x0044: cmdName = "SafetyStatus / SafetyAlert"; break;
        case 0x0045: cmdName = "AFE Data"; break;
        case 0x0050: cmdName = "SafetyAlert"; break;
        case 0x0051: cmdName = "SafetyStatus"; break;
        case 0x0052: cmdName = "PFAlert"; break;
        case 0x0053: cmdName = "PFStatus"; break;
        case 0x0054: cmdName = "OperationStatus"; break;
        case 0x0055: cmdName = "ChargingStatus"; break;
        case 0x0056: cmdName = "GaugingStatus"; break;
        case 0x0057: cmdName = "ManufacturingStatus"; break;
        case 0x0058: cmdName = "AFE Register"; break;
        case 0x0059: cmdName = "MaxTurboPwr()"; break;
        case 0x005A: cmdName = "SusTurboPwr()"; break;
        case 0x005B: cmdName = "TURBO_PACK_R"; break;
        case 0x005C: cmdName = "TURBO_SYS_R"; break;
        case 0x005D: cmdName = "TURBO_EDV"; break;
        case 0x005E: cmdName = "MaxTurboCurr()"; break;
        case 0x005F: cmdName = "SusTurboCurr()"; break;
        case 0x0060: cmdName = "Lifetime Data Block 1"; break;
        case 0x0061: cmdName = "Lifetime Data Block 2"; break;
        case 0x0062: cmdName = "Lifetime Data Block 3"; break;
        case 0x0063: cmdName = "Lifetime Data Block 4"; break;
        case 0x0064: cmdName = "Lifetime Data Block 5"; break;
        case 0x0070: cmdName = "ManufacturerInfo"; break;
        case 0x0071: cmdName = "DAStatus1"; break;
        case 0x0072: cmdName = "DAStatus2"; break;
        case 0x0073: cmdName = "GaugeStatus1"; break;
        case 0x0074: cmdName = "GaugeStatus2"; break;
        case 0x0075: cmdName = "GaugeStatus3"; break;
        case 0x0076: cmdName = "CBStatus"; break;
        case 0x0077: cmdName = "State-of-Health"; break;
        case 0x0078: cmdName = "FilteredCapacity"; break;
        case 0x00F0: cmdName = "IATA_SHUTDOWN"; break;
        case 0x00F1: cmdName = "IATA_RM"; break;
        case 0x00F2: cmdName = "IATA_FCC"; break;
        case 0x0F00: cmdName = "ROM Mode"; break;
        case 0x3008: cmdName = "WRITE_TEMP"; break;
        case 0xF080: cmdName = "Exit Calibration Output Mode"; break;
        case 0xF081: cmdName = "Output CCADC Cal"; break;
        case 0xF082: cmdName = "Output Shorted CCADC Cal"; break;
        default: {
          char buf[7];
          sprintf(buf, "%04X", subcmd);
          cmdName = "Unknown MA Subcommand 0x" + String(buf);
          break;
        }
      }
    }
    ...
    if (i2cError) recoverI2C();
        ...
        // Prepend command name to response
    response = "Name:" + cmdName + "\n";
